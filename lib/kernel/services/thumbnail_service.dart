...
class ThumbnailService {
  ...
  ThumbnailProvider get _providerImpl {
    ...
    final created = switch (defaultTargetPlatform) {
      ...
      // B1 中间态保留 provider 内建磁盘缓存注入；B2 迁移到 Service 层 DiskCache。
  ...
  Future<ImageProvider?> _getThumbnailImpl(String filePath) async {
    ...
    final provider = await _providerImpl.getThumbnail(identity.path);
    if (provider != null) {
      _cachePutImpl(identity.path, key, provider);
    }
    return provider;
  }

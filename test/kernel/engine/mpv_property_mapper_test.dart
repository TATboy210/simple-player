...
void main() {
  group('mapVideoEffect', () {
    test('0 = 中性 → mpv 0', () {
      final p = MpvPropertyMapper.mapVideoEffect(VideoEffectType.brightness, 0.0);
      expect(p.key, 'brightness');
      expect(p.value, '0');
    });

    test('边界 clamp — ±1 → ±100, 超界不越', () {
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.brightness, 1.0).value, '100');
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.brightness, -1.0).value, '-100');
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.contrast, 2.0).value, '100');
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.saturation, -3.0).value, '-100');
    });

    test('四键映射到对应 mpv 属性', () {
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.brightness, 0.5).key, 'brightness');
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.contrast, 0.5).key, 'contrast');
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.saturation, 0.5).key, 'saturation');
      expect(MpvPropertyMapper.mapVideoEffect(VideoEffectType.hue, 0.5).key, 'hue');
    });
  });
  ...
  group('mapDelay', () {
    test('毫秒 → 秒 3 位小数, 支持负值', () {
      ...
      expect(MpvPropertyMapper.mapDelay('audio-delay', 1500).key, 'audio-delay');
    });
  });

  group('mapAspectRatio', () {
    test('keepOriginal → no', () {
      final p = MpvPropertyMapper.mapAspectRatio(AspectRatioMode.keepOriginal.mdkValue);
      expect(p, isNotNull);
      expect(p!.value, 'no');
    });

    test('标准比率 → mpv 比率语法', () {
      expect(MpvPropertyMapper.mapAspectRatio(AspectRatioMode.ratio4_3.mdkValue)!.value, '4:3');
      expect(MpvPropertyMapper.mapAspectRatio(AspectRatioMode.ratio16_9.mdkValue)!.value, '16:9');
      expect(MpvPropertyMapper.mapAspectRatio(AspectRatioMode.ratio21_9.mdkValue)!.value, '21:9');
    });

    test('stretch/cropFill — 单属性不可表达, 返回 null (诚实降级)', () {
      expect(MpvPropertyMapper.mapAspectRatio(AspectRatioMode.stretch.mdkValue), isNull);
      expect(MpvPropertyMapper.mapAspectRatio(AspectRatioMode.cropFill.mdkValue), isNull);
    });

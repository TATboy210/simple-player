...
void main() {
  ...
  group('aspect ratio integration (WP-01..WP-04)', () {
    late List<double> appliedRatios;

    setUp(() {
      appliedRatios = [];
      AspectRatioService.setForTest((ratio) async {
        appliedRatios.add(ratio);
      });
    });

    tearDown(() {
      AspectRatioService.resetForTest();
    });
    ...
    test('playing locks aspect ratio to video ratio', () async {
      ...
      expect(appliedRatios, isNotEmpty);
      expect(appliedRatios.last, closeTo(2.35, 0.01));
    });
    ...
    test('stopped unlocks aspect ratio', () async {
      ...
      expect(appliedRatios, isNotEmpty);
      expect(appliedRatios, contains(0.0));
    });
    ...
    test('playing with zero ratio does not call matchVideo', () async {
      ...
      final positiveCalls = appliedRatios.where((r) => r > 0);
      expect(positiveCalls, isEmpty);
    });
    ...
    test('rapid state changes maintain correct ratio', () async {
      ...
      final unlockCalls = appliedRatios.where((r) => r == 0.0);

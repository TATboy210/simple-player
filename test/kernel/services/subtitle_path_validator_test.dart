...
void main() {
  group('SubtitlePathValidator', () {
    ...
    test(
      'rejects paths that cannot be passed safely to the subtitle engine',
      () {
        const paths = <String>[
          '',
          ' \t ',
    ...
    test(
      'accepts only existing regular files within the configured size limit',
      () async {
        ...
        final subtitle = File(
          '${directory.path}${Platform.pathSeparator}movie.srt',
        );
        ...
        expect(
          await SubtitlePathValidator.isLoadableLocalFile(subtitle.path),
          isTrue,
        );
        expect(

...
///    03-UAT Test 1 归档佐证);
/// D) close-advance 路径验证 dismiss 推进零第二报告入队、抛错
///    presentation 监听被 FlutterError.onError 边界收容(effect 按契约
///    只在 intake 接纳(newReport/merged)时触发,dismiss 不经过
///    effect 链 —— 既有 :743 只覆盖 intake,dismiss 隔离此前无用例
///    显式锁死,条件补差命中)。
///
...
void main() {
  ...
  group('混合爆发有界性(VER-02, D-03/D-04)', () {
    test('keeps a 1000 mixed burst bounded with visible merge counts and zero '
        'unhandled errors', () {
  ...
  group('爆发下写盘受控(VER-02, D-03/D-04)', () {
    test(
      'keeps the single-writer chain intact with ordered packs under burst',
      () async {
  ...
  group('pump 响应不卡(VER-02, D-03)', () {
    test(
      'completes pre-scheduled timers and microtasks on time after burst',
      () {
        fakeAsync((async) {
  ...
  group('关闭失败隔离(VER-03 条件补差)', () {
    test('keeps close-advance contained when the effect fails', () {
      ...
      subject.reportPlatformSafely(
        StateError('close-head'),
        stack('close.dart'),
      );
      subject.reportPlatformSafely(
        StateError('close-next'),
        stack('close.dart'),
      );
      ...
      // 保持 isReady 语义),随后关闭推进 —— 验证 dismiss 推进零第二
      // 报告入队、抛错 presentation 监听被 FlutterError.onError 边界
      // 收容(effect 按契约只在 intake 接纳时触发,dismiss 不经过
      // effect 链,故 close 路径的隔离口径不含 effect)。

...
void main() {
  ...
  group('Settings Tab Content', () {
    ...
    testWidgets(
      'GeneralTab (index 0) renders 语言 section and 界面语言 SettingRow',
      (tester) async {
        // Arrange & Act
    ...
    testWidgets(
      'switching to EqualizerTab (index 1) renders 均衡器 section',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'switching to AudioTab (index 2) renders 音频输出 section',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'switching to VideoTab (index 3) renders 解码器 section',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'switching to ShortcutsTab (index 4) renders 快捷键 section',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'switching to AboutTab (index 5) renders 关于 section',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'switching to PerformanceTab (index 6) renders 性能监控 section',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'GeneralTab contains Switch control for 深色模式',
      (tester) async {
        // Arrange & Act
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'AudioTab contains Slider for volume control',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'all tabs use SettingRow for layout',
      (tester) async {
        // Arrange & Act
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'tab content uses AnimatedSectionList for staggered fade-in',
      (tester) async {
        // Arrange & Act
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'IndexedStack keeps all 7 tab widgets alive after switching',
      (tester) async {
        // Arrange
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'shell still renders 7 SettingsNavItem tabs after wiring',
      (tester) async {
        // Arrange & Act
        final (controller, _) = await pumpShell(tester);
        controller.open();
    ...
    testWidgets(
      'button bar still renders after wiring tab content',
      (tester) async {
        // Arrange & Act
        final (controller, _) = await pumpShell(tester);
        controller.open();

...
| 需求 | 标准 | 验证方式 |
|------|------|----------|
| R2-1 | PositionPoller 静默模式 500ms | 单元测试 + 代码审查 |
...
- **不添加 resize 暂停** — PositionPoller 在 resize 期间继续轮询（D-02 决策）

**D-02: 只冻结 UI 层，不暂停 PositionPoller**
- PositionPoller 在 resize 期间继续轮询（FFI 开销极低，ns 级）
- ProgressBar/OsdOverlay/TimeRangeDisplay 在 `isResizing=true` 时跳过 rebuild
- UI 层通过 `AnimatedBuilder` 包裹，resizing 时返回缓存帧
- resize 结束后自然读取最新位置，无跳变（已验证：Pitfall 1 解决方案）

**D-03: 扩展已有 isResizing 信号链**
- 不新建 resize 检测机制，复用 `WindowService.isResizing`
- 信号路径: WindowService → PlayerScreen → ControlsOverlay → ProgressBar/OsdOverlay
...
- `lib/kernel/engine/fvp_engine.dart` (724 行) — 调用 startSilent()
...
2. 新增 `startSilent()` — 启动轮询 + 延迟 3 秒进入静默模式:
   - 调用 `start()` 启动 250ms 轮询
   - 设置 Timer 3 秒后调用 `_updateInterval(_silentPollMs)`
3. 修改 `seeking` setter — seek 完成后重置静默模式 Timer:
   - 在 `setActive()` 调用后，取消静默 Timer 并重新设置 3 秒延迟
4. **不添加 `resizing` setter** — PositionPoller 在 resize 期间继续轮询（D-02 决策）

### fvp_engine.dart

1. 修改 `play()` 方法（第 413-424 行）— 使用 `_positionPoller.startSilent()` 替代 `_positionPoller.start()`:
   - 视频播放时自动进入静默模式（500ms），减少 FFI 调用
   - seek 后自动恢复快速轮询（已有逻辑）
2. **不添加 pausePolling/resumePolling** — PositionPoller 不暂停（D-02 决策）

**测试**:
- `test/kernel/engine/position_poller_test.dart` — 新增测试:
  - `startSilent()` 启动后 3 秒切换到 500ms 间隔
  - seeking 期间静默模式 Timer 重置
  - `play()` 后轮询间隔从 250ms 变为 500ms（验证 fvp_engine 集成）
...
- `lib/ui/player/player_screen.dart` — 传递 isResizing 到 ControlsOverlay
...
1. 确保 ControlsOverlay 已传递 resizing（当前第 279 行已有）
2. **不调用 engine.pausePolling/resumePolling** — PositionPoller 不暂停（D-02 决策）
...
Task 1 先完成（PositionPoller 静默模式），Task 2 独立于 Task 1（UI 层冻结不依赖 PositionPoller API）。
...
1. **CPU 测量** — DevTools CPU Profiler 确认 resize 期间 CPU 下降 >30%
2. **进度条连续性** — resize 结束后进度条位置正确（无跳变）
3. **OSD 响应** — resize 期间按音量键，resize 结束后 OSD 显示
4. **全量测试** — `flutter test` 全部通过

**如果 CPU 下降未达 30%:**
- 检查 ProgressBar/OsdOverlay 是否正确跳过 rebuild
- 检查 isResizing 信号是否正确传递
- 考虑是否需要扩展到更多 UI 组件

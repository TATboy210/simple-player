...
## 遗留（用户决定暂缓, 2026-08-23）

- **Windows 风格标题栏闪现**（全屏/退出全屏的窗口变换过程中仍可见）:
  根因=media_kit 退出时先恢复 WS_CAPTION 样式再缩回几何, 中间帧 DWM
  按新样式画原生标题栏; 全局 DWMNCRP_DISABLED 方案（`184dc804`）效果
  不理想已按用户要求撤回（`622eece5`）。**用户决定暂不处理。**
  后续若重启该问题, 候选: 仅在样式切换过渡窗口期临时禁用 DWM 非客户区
  渲染（窗口态外观零影响）— 不要直接重提全局禁用方案。

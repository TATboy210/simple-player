...
- [ ] 04-01-PLAN.md — 贯通 settings.json → 三层位置链 → sink 激活 → 诊断包落盘的端到端 tracer，落地便携设置存储与 exe 根优先位置链（SET-02/SET-03，D-01/D-02）。

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 04-02-PLAN.md — 日志路径单层校验 + DiagnosticLogTarget 重定向协调器 + 回退双通道告知（SET-02，D-03/D-04）。
